...
test('index publica v3.24.0 sin caché antigua y documenta el tamaño del borrador', () => {
  const html = fs.readFileSync(path.resolve(__dirname, '..', 'index.html'), 'utf8');
  assert.match(html, /class="topbar__badge">v3\.24\.0</);
  assert.match(html, /css\/styles\.css\?v=3\.24\.0/);
  assert.match(html, /src\/js\/app\.js\?v=3\.24\.0/);
  assert.match(html, /src\/js\/building\.js\?v=3\.24\.0/);
  assert.match(html, /src\/js\/garden\.js\?v=3\.24\.0/);
  assert.match(html, /src\/js\/config\.js\?v=3\.24\.0/);
  ...
  assert.match(html, /src\/js\/solid\.js\?v=3\.24\.0/);
  assert.match(html, /src\/js\/airbrush\.js\?v=3\.24\.0/);
  assert.match(html, /src\/js\/hatch\.js\?v=3\.24\.0/);
...
test('«Abrir proyecto» dice lo que abre y dónde lo abre', () => {
  ...
  // hacía el botón (v2.42.0). Desde la v3.18.1 abre en pestaña nueva, y el
...
/* ────────────────────────────────────────────────────────────
   Aspecto de boceto (v3.13.4): los mandos y sus catálogos
   ──────────────────────────────────────────────────────────── */
