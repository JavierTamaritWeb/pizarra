...
Only `FILLABLE_TYPES` (rect, roundedRect, circle) take a fill; UI components carry their own as part of their design. Three fields control it: `fill` (boolean, on/off), the optional `fillColor` (hex), and the optional `fillTransparent` (boolean; `true` means ~40% opacity). They are deliberately independent — the "Rellenar formas" checkbox toggles `fill` only and never touches `fillColor` or `fillTransparent`, so emptying and refilling a shape recovers the same color and opacity mode. "Relleno translúcido" has the standard dual panel semantics: with a selection it updates the selected fillable shapes in one undo step; without a selection it sets the creation default. Solid mode is represented by the absence of `fillTransparent`, preserving backward compatibility.

`Renderer.fillStyle(el)` is the single place that resolves these fields: transparent mode uses `(el.fillColor || el.color) + '66'`; otherwise it uses `el.fillColor || el.color + '20'`. Therefore **a shape without `fillColor` keeps the classic translucent stroke tint**, and projects saved before fill had their own color render identically — preserve that fallback. SVG and HTML exports mirror this resolution. `isValidElement` validates `fillTransparent` as a boolean and `fillColor` as hex because both affect serialized/exported output.
