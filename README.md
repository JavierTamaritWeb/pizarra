...
[![Versión](https://img.shields.io/badge/versión-3.21.0-blueviolet?style=flat-square)](CHANGELOG.md)
...
[![Tests](https://img.shields.io/badge/tests-987%20unitarios%20%2B%20210%20e2e-brightgreen?style=flat-square)](#tests)
...
| **Formas** | Rectángulo, redondeado, elipse, cuadrado, trapecio, triángulo, pentágono, hexágono y las **estrellas de 5 y 6 puntas**. Los polígonos regulares se arrastran desde el centro y conservan sus lados iguales; el trapecio admite proporciones libres. El **triángulo irregular** también llena la caja arrastrada, y en sus ajustes se elige **isósceles** (2 lados iguales) o **escaleno** (3 lados distintos). |
...
| **Marcos** | El contenedor de una pantalla, con su rótulo. Al elegir la herramienta se escoge el **dispositivo** (móvil, tablet o escritorio): un clic coloca el marco a esa medida exacta y el arrastre sigue dibujando libre. Se coge por el **borde** o por el rótulo —nunca por dentro, así que sigues dibujando y seleccionando dentro de él— y **al moverlo se lleva lo que contiene**; al estirarlo, no: entra o sale contenido, que es para lo que sirve. Con el marco seleccionado, «Exportar → Solo la selección» **recorta a su caja exacta**: la forma de sacar una pantalla suelta de un lienzo con varias. |
...
| **Puntas de flecha** | Clásica, **maciza**, **barra** —el remate de cota de un plano—, **punto** o **media** —un ala sola, el rasgo de un 1 escrito a mano; en las curvas cae del lado de la comba, continuando el giro del trazo—, en los dos extremos cuando la flecha es de doble punta. |
...
| **Semicírculos** | 180° exactos y sin puntas. El arrastre fija el diámetro; después `+`/`−` o su handle ajustan el radio manteniendo la media circunferencia. `Q` convierte una flecha curva en semicírculo y viceversa. La **Flecha semicírculo** (⤾) es el mismo arco pero con punta —nace donde acaba el arrastre, `D` la invierte, «Doble punta» pone las dos y la forma se elige entre Clásica, Maciza, Barra, Punto o Media—. |
...
| **Componentes UI** | Botón, input, imagen, navbar y tarjeta, con etiquetas editables (doble clic) y, desde la v3.23.0, **catálogo de variantes**: botón primario, secundario, fantasma o con icono; input de texto, búsqueda, área de texto o con error; tarjeta con imagen arriba, horizontal o solo texto; navbar con enlaces, buscador o avatar. La herramienta **Piezas** reúne las menores —avatar, barra de progreso, migas de pan, globo, insignia y paginación—, cada una con su caja natural a un clic; el **globo** elige además su forma (redondeado, bocadillo o nube de pensamiento) y el lado del pico (abajo, arriba, izquierda o derecha). |
...
| **Texto en varias filas** | En el editor de texto, `Enter` hace un salto de línea, como en Word (v3.30.0), y se termina con `Esc`, con el botón «✓ Listo» que cuelga del editor, con `Ctrl`/`Cmd`+`Enter` o con un clic fuera (v3.30.1); ninguna de esas salidas pierde lo escrito. El editor crece con lo escrito y no envuelve, así que se ve tal como se va a pintar. En los rótulos de una línea (botón, input, flecha…) `Enter` confirma. |
| **Texto dentro de las formas** | Doble clic sobre cualquier forma —rectángulo, redondeado, óvalo, trapecio, triángulos, polígonos regulares y estrellas— y el texto queda **dentro**, repartido en líneas al ancho de la caja inscrita de esa figura y centrado, **como en Word** (v3.28.0); si no cabe, la letra se reduce sola hasta entrar entera. Se edita también desde el campo «Texto» del panel, viaja con la forma al moverla, escalarla o girarla, y sale igual en el SVG y el HTML exportados. Desde la v3.29.0 tiene **estilo propio** en la sección «Texto»: tamaño, negrita, **color**, **letra** (la del lienzo o cualquiera de las siete, forma a forma) y **alineación** horizontal y vertical dentro de su caja. |
...
| **«Select», solo selección** | El clic selecciona cualquier elemento (el grupo completo; doble clic desciende a la pieza) y el arrastre dibuja **siempre** marquesina, incluso empezando encima de un elemento — el gesto que con Mover lo desplazaría. Nada se mueve jamás con ella: en un lienzo denso se enmarca sin miedo y el panel edita lo seleccionado como siempre. Desde la v3.26.0 un marco que cae **entero dentro de una imagen** separa esa región como pieza propia (la foto queda con el hueco transparente; Deshacer lo revierte en un paso). |
| **Borrador real** | Borra **lo que se ve**, no la caja: una forma sin relleno se borra por su contorno, así que barrer entre las ventanas de una fachada no se lleva el muro. Y **muerde en vez de fulminar**: recta, flecha, lápiz, flecha curva, aerógrafo y el contorno de las formas sin relleno se **recortan** por donde pasas —cruzar una línea por la mitad, o por donde se cruzan dos trazos, solo borra ese tramo; una mancha de espray se parte en dos—. El **texto, los emojis, las imágenes, los componentes y las formas rellenas** no tienen trazo que recortar, así que lo que queda de ellos se convierte en imagen: se ve igual y conserva el mordisco, a cambio de dejar de editarse como texto, componente o forma. Si el barrido no quita un solo píxel no pasa nada, ni siquiera un paso de deshacer: cruzar el hueco vacío de una tarjeta ya no se la lleva. Nada desaparece entero por un roce. Lo borrado no reaparece al mover el dibujo ni viaja oculto dentro del archivo exportado, y cada pasada se deshace como una sola acción. Tamaño ajustable de 4 a 100 px: al elegir la herramienta se abre un modal con previsualización, y volver a pulsarla lo reabre sin salir de ella. |
...
**Edificios** — planta, fachada, tejado, puerta, ventana, balcón, muro, verjas, cancela e iluminación. La fachada abre un modal con **miniatura en vivo**, tres vistas y los ajustes de plantas, ventanas por planta, pendiente y cubierta, todos sincronizados con el panel lateral. El balcón trae **8 tipos** y su catálogo, como los del jardín, usa **el dibujo real como icono**. El muro se dibuja en **vista de planta o de alzado**, en piedra, hormigón o ladrillo cara vista, con verja opcional arriba y **dieciocho cancelas** a elegir. La herramienta **Verjas** dibuja paños independientes en planta o alzado, ofrece trece diseños de forja con lanzas —incluidos seis inspirados en tradiciones españolas— y regula su altura entre **0 y 350 cm** mediante una miniatura en vivo. Los trece remates tienen hojas diferentes —lanceolada, aguja, ojiva, rombo, laurel, llama, palmeta, hoja facetada, flor de lis, piramidión, cáliz, corazón y rocalla— sin abandonar el repertorio clásico de los maestros forjadores. La herramienta **Cancela** permite colocar cualquiera de los dieciocho estilos como elemento autónomo, en planta o alzado y con altura regulable de **0 a 350 cm**. La herramienta **Iluminación** añade **8 modelos** de alumbrado —farola de pie, de doble foco, de forja, de forja de doble foco, de pared, foco, foco con mástil y torre de luz—, con la misma clase de catálogo dibujado con la geometría real. Los dos de **alumbrado deportivo** (foco con mástil y torre de luz) se acotan en metros: se elige la **actividad** —pádel, polideportivo, piscina, fútbol sala, fútbol 11, atletismo, rugby, béisbol o estadio—, que propone su **altura de montaje recomendada**, y un deslizador de **4 a 45 m** la ajusta avisando si se sale del rango habitual. Ahí la altura sale de la cota y no del arrastre, y la pieza se planta por su base.

Desde la v3.22.0 la sección también dibuja **la parte técnica del plano**: **Escalera** (recta, en L, en U, de caracol y rampa, en planta —con línea de rotura y flecha de subida— o alzado), **Cota** (línea con remates de barra, testigos y la **cifra en metros** calculada según una escala normalizada 1:20, 1:50, 1:100 o 1:200), **Símbolos** (flecha de norte, escala gráfica coherente con esa misma escala, marca de corte A–A′ y cota de nivel con su cifra), **Pilar** (secciones cuadrada, circular y con achurado en planta, y columna clásica de alzado con basa y capitel), **Mobiliario** (ocho piezas en planta: inodoro, lavabo, bañera, ducha, cocina, cama, mesa con sillas y sofá), **Porche** (porche, pérgola y marquesina en alzado) y **Siluetas** (persona y coche que un clic planta a su **medida real** según la escala activa). El **Tejado** admite además un **complemento** —chimenea, buhardilla o lucernario— elegido en un select de su catálogo.
...
| **Iluminación** | Farola de pie · de doble foco · de forja · de forja de doble foco · de pared · foco · foco con mástil · torre de luz |
...
| **Decoración** (`Z`) | Maceta · pozo · regadera · piedra · banco · fuente · reloj de sol (de suelo o de pared) · estanque · **piscina** · **flecha de norte** · **escala gráfica**, que dibuja tramos de un metro a la escala activa del jardín |
...
- **Papel de plano de obra, y cinco aspectos en un clic**: el lienzo nace con fondo de pizarra azulada y la cuadrícula casi blanca encima, y la fila **«Aspecto»** cambia papel, color de rejilla y rejilla sí/no de una vez — **Plano**, **Blanco**, **Milimetrado**, **Crema** y **Pizarra**—; los dos colores siguen siendo personalizables a mano y todo persiste entre sesiones. Un aspecto describe el papel y nunca toca el color de la tinta (en «Pizarra» hay que elegir un color claro, como en un encerado de verdad). Son ajustes **de pantalla**: no viajan a ningún dibujo exportado, así que lo que se imprime sale siempre sobre blanco limpio y sin rejilla. Y **«Limpiar todo» respeta el aspecto**: vacía el dibujo y devuelve el resto de ajustes a fábrica, pero el papel que hayas elegido se queda puesto. El aspecto sí viaja en el **proyecto JSON** —el único formato que se vuelve a abrir—, así que un dibujo hecho sobre «Pizarra» con tinta clara se recupera como se dibujó.
- **Pestañas de pizarra** (v3.17.0): varias pizarras abiertas a la vez, cada una con su dibujo, su historial de deshacer, su zoom y su aspecto de lienzo. Siguen la nomenclatura **«Pizarra N - nombre»** (el número es la posición y se renumera solo; el nombre es un texto corto que se edita con doble clic). Se crean con «+» y se cierran con «×» (preguntando si tienen dibujo); al reabrir la app **vuelven todas**, con la activa recordada. «Abrir proyecto» abre el `.json` en su propia pestaña, con su nombre.
...
- **Cada herramienta abre sus ajustes al pulsarla**, como ya hacían el Borrador y los catálogos. Lápiz, Línea, Flecha, Flecha curva, Semicírculo y Flecha semicírculo muestran grosor, color, discontinuo, doble punta, el **trazo con presión** del lápiz y la **curvatura** con la que nacen las flechas curvas —del 0 % (una recta con punta) al 100 % de la distancia entre sus extremos, cuando antes era siempre la misma—; las **once formas** añaden el relleno entero —rellenar, translúcido, opacidad y color— y, las que guardan su orientación como ángulo, un **giro** con el paso propio de cada una (45° el cuadrado, 36° el pentágono y la estrella de 5 puntas, 30° el hexágono y la de 6…), de modo que la forma nace ya orientada en vez de tener que dibujarla y girarla a golpe de clic. **Texto** ajusta tamaño de letra, grosor y color; **Imagen, Pestañas y Menú lateral** comparten un modal con color y grosor; **Botón, Input, Tarjeta, Navbar, Diálogo, Formulario, Tabla, Gráfico y Piezas** eligen su **variante en un catálogo** pintado con el renderer real (primario/secundario/fantasma/con icono, texto/búsqueda/área/con error, casilla/radio/interruptor/desplegable/deslizador, barras/líneas/tarta/indicador/anillo/área…), y su rótulo se edita con la pieza seleccionada desde el ⚙ de «Posición y tamaño»; el **Marco** abre su selector de dispositivo; el catálogo de **Emoji** elige también el tamaño (32–96 px), independiente del de letra. Todos llevan **muestra en vivo** dibujada con el renderer real. Cerrar deja la herramienta lista, y volver a pulsarla reabre sus ajustes. En el panel, **cada sección con ajustes propios lleva su ⚙** —«Posición y tamaño», «Relleno», «Texto», «Edificios», «Jardín» y «Elementos»—, y cada uno abre siempre los de esa sección, sin cambiar de herramienta ni soltar la selección. El de «Posición y tamaño» sigue al **tipo de lo seleccionado**, no a la herramienta activa: con un botón seleccionado abre los del componente aunque estés en Mover. Los mismos ajustes siguen en el panel, sincronizados; las únicas herramientas sin modal son Mover y «Select», que no tienen ajustes propios que enseñar.
...
- **Barras de herramientas flotantes**: el botón **«Barras»** desglosa el lateral en **seis paletas arrastrables** —Edición, Dibujo, Formas y 3D, UI, Edificios y Jardín— que se llevan a donde estorben menos y se pliegan por su asa. Al activarlo aparecen **apiladas a la izquierda**, en el sitio de siempre; para **recoger una sola** se arrastra de vuelta a la franja izquierda —que se resalta al entrar— y vuelve a su hueco de la columna **sin mover las demás**, mientras que el botón las devuelve todas. El modo se recuerda entre sesiones; las posiciones, a propósito, no. En pantallas estrechas (donde el panel es un cajón) manda siempre la barra lateral.
...
1. Elige una herramienta en la barra lateral —**Mover** abre la lista, arriba del todo— o con su atajo de teclado. Si prefieres tenerlas a mano sobre el lienzo, el botón **Barras** del topbar desglosa la barra en **seis paletas flotantes** arrastrables y plegables (al recargar vuelven a su sitio de fábrica).
...
| Atajo | Acción |
| --- | --- |
| `P` `L` `A` `U` `G` | Lápiz · Línea · Flecha · Flecha curva · Semicírculo (el Aerógrafo, la Tinta y la Flecha semicírculo van sin atajo) |
...
| `T` `J` `B` `I` `M` `N` `K` | Texto · Emoji · Botón · Input · Imagen · Navbar · Tarjeta (Marco, Formulario, Tabla, Gráfico, Diálogo, Pestañas, Menú lateral y Piezas van sin atajo) |
| `V` `E` | Edición: Mover / seleccionar · Borrador |
| `W` `1` `2` `0` `Y` | Edificios: Planta · Fachada · Tejado · Puerta · Ventana (Balcón, Muro, Verjas, Cancela, Iluminación, Escalera, Cota, Símbolos, Pilar, Mobiliario, Porche y Siluetas no tienen atajo) |
...
Las herramientas que abren catálogo (Edificios y Jardín) muestran su modal de tipos al pulsar el atajo. **Aerógrafo**, **Tinta**, **Flecha semicírculo**, **Select**, **Marco**, las siete piezas de formulario, datos y remates, **Balcón**, **Muro**, **Verjas**, **Cancela**, **Iluminación**, las siete de la parte técnica del plano, **Caminos**, **Aromáticas**, **Trepadoras** y las cuatro de 3D no tienen atajo: ya no queda ninguna tecla suelta libre.
...
└── js/
    ...
    ├── trapezoid.js     Geometría y rotación del trapecio y del triángulo irregular
...
**987 tests unitarios** con el runner nativo de Node, sin ninguna dependencia
de runtime:
...
**210 tests end-to-end** en un navegador real (Playwright), para lo que un stub no puede juzgar: layout, CSS, foco, acciones por defecto del navegador, la navegación del lienzo (zoom al cursor, pan, encuadres), el feedback en vivo, los gestos multitáctiles (despachados por CDP), los flujos completos de Verjas y Cancela y el Jardín botánico en escritorio, móvil y anchos intermedios.
